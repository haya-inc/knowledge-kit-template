---
name: url-ingest
description: ユーザーが chat で渡してきた URL を inbox/web/<slug>.url に落とし込み、source/web/<slug>.md まで変換するためのスキル。プレビュー取得 → canonical URL 決定 → slug/frontmatter をユーザーと確認 → .url 生成 → Markdown 変換 → 関連リンクの選別取り込みまでを扱う。ユーザーが「これソース化して」「取り込んで」「入れといて」等と URL を添えて依頼したとき、あるいは URL 単独発話が届いたときに使う。inbox に既に .url が置かれている場合は代わりに sync スキルを使う。
---

# url-ingest — chat で受けた URL を inbox に落とし込む

ユーザーが会話の中で URL を渡してきたとき、それを取り込み対象に
したいかを確認しつつ、`inbox/web/<slug>.url` を生成して
通常の sync に合流させるためのスキルです。
...
- そのまま sync スキル (`.claude/skills/sync/SKILL.md`) の手順に
  合流し、`source/web/<slug>.md` までを一気通貫で仕上げる
- 取り込んだ URL の関連リンクを列挙し、ユーザーに選んでもらって
  まとめて同じフローに流す

inbox に既に `.url` が置かれていて sync だけ走らせたい場合は sync
スキルのほうが担当です。このスキルは chat 経由の URL 専用です。
...
- 逆に「この URL の中身を教えて」のような「今回は読むだけ」と
  読める問い合わせは、このスキルに入らずその場で答える

## 前提

- Python 3.10+ と PyYAML、`tools/knowledgekit.py` が動くこと
...
sync スキル (`.claude/skills/sync/SKILL.md`) の 1. scan → 2. 変換
→ 3. record を、この 1 ファイル分だけ実行します。

1. `python tools/knowledgekit.py scan --json` — 先ほど書いた `.url` が
   `added` として現れる
...
sync スキルの 4. 以降と同じです。関連リンクをバッチ処理する場合は
...
11. で選ばれた URL については、本スキルの 1. から繰り返します。
...
「もう一段深く」と明示したときに限り、本スキルをもう一度起動して
進めます。
